...
from speaches.executors.mlx_whisper.utils import model_registry as mlx_whisper_model_registry
...
def list_local_models(task: ModelTask | None = None) -> JSONResponse:
    ...
    if task is None or task == "automatic-speech-recognition":
        ...
        models.extend(list(mlx_whisper_model_registry.list_local_models()))
...
def download_remote_model(model_id: ModelId) -> Response:
    ...
    elif model_id in [model.id for model in mlx_whisper_model_registry.list_remote_models()]:
        was_downloaded = mlx_whisper_model_registry.download_model_files_if_not_exist(model_id)
    else:
        raise HTTPException(status_code=404, detail=f"Model '{model_id}' not found")
...
def get_remote_models(task: ModelTask | None = None) -> JSONResponse:
    ...
    if task is None or task == "automatic-speech-recognition":
        ...
        models.extend(list(mlx_whisper_model_registry.list_remote_models()))
    return JSONResponse(content={"data": [model.model_dump() for model in models], "object": "list"})
